...
def go(args):
    ...
    run = wandb.init(job_type="train_random_forest", name=f"Run_{timestamp}")
    ...
    # Create an artifact for model export
    model_artifact = wandb.Artifact(args.output_artifact, type="model_export",\
        description="Random Forest Model Export")
    model_artifact.add_dir("random_forest_dir")
    model_artifact.metadata = rf_config

    run.log_artifact(model_artifact)
    fig_feat_imp = plot_feature_importance(sk_pipe, processed_features)

    # Save r_squared under the "r2" key in run summary
